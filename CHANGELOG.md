...
- 新增 JPG／PNG 合成 PDF，支援圖片排序、原圖尺寸、A4 與頁邊距。
- 修正 PDF 儲存／頁面重建期間切換文件造成的資料錯置；非同步操作鎖定編輯並依序處理外部開檔。
- 修正 OCR 暫存檔覆寫既有文字檔及 Office 失敗清理誤刪小檔案；OCR 結果以排他方式建立。
- 修正 PDF 密碼經空白正規化或引號轉義後漏入診斷紀錄的問題。
- CSV 匯出將可能被試算表解讀為公式的儲存格轉為文字；PDF 簽名匯入明確限制為 PNG／JPEG。
- 本機 HTTP 服務拒絕不受信任 Host、跨來源 token 請求與異常 URL；錯誤請求不再終止服務。
- 更新 Electron 至 43.6.0、electron-builder 至 26.15.3，CI 使用 Node.js 24。
- 修正 Windows 工具解壓流程，排除打包暫存紀錄，並加強必要引擎、產物內容及轉換結果檢查。
- 修正 Tesseract 設定目錄未被複製，令內附引擎無法建立可搜尋 PDF；打包驗證新增 PDF 設定與字型檢查。
- Windows 打包明確使用正常壓縮等級，並清除中斷建置留下的 NSIS 壓縮暫存，避免重試沿用損壞半成品。
- 大型 Windows 產物採分段讀取及雜湊驗證，避免驗證器一次載入完整 EXE 造成記憶體不足。
- Python 測試隔離工作紀錄，避免影響本機使用資料。
...
- 修正 Linux／macOS CI 無法執行 `7zip-bin` 以驗證 release payload 的權限問題。
