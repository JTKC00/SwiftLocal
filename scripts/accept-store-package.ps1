...
if (!$Package) { $Package = Join-Path $root 'dist-store-test/SwiftLocal-0.4.1-store-TEST-x64.appx' }
$Package = (Resolve-Path $Package).Path
$identity = 'SwiftLocal.StoreSpike.TEST'
if (Get-AppxPackage -Name $identity) { throw 'Existing TEST package found; refusing to replace user state' }
...
try {
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Developer TEST signing failed' }
  & $sdk.FullName verify /pa $signed
  if ($LASTEXITCODE -ne 0) { throw 'Developer TEST signature verification failed' }
